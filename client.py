...
class Client:
    def __init__(self):
        ...
        self._global_headers = {
            'content-type': '',
            'user-agent' : 'RealmoceanPythonSDK/4.1.0 (${os.uname().sysname}; ${os.uname().version}; ${os.uname().machine})',
            'x-sdk-name': 'Python',
    ...
    def set_project(self, value):
        ...
        self._global_headers['x-realmocean-project'] = value
        return self

    def set_key(self, value):
        """Your secret API key"""

        self._global_headers['x-realmocean-key'] = value
        return self

    def set_jwt(self, value):
        """Your secret JSON Web Token"""

        self._global_headers['x-realmocean-jwt'] = value
        return self

    def set_locale(self, value):
        self._global_headers['x-realmocean-locale'] = value
        return self
    ...
    def call(self, method, path='', headers=None, params=None):
        ...
        try:
            ...
            if response != None:
                content_type = response.headers['Content-Type']
                if content_type.startswith('application/json'):
                    raise RealmoceanException(response.json()['message'], response.status_code, response.json().get('type'), response.json())
                else:
                    raise RealmoceanException(response.text, response.status_code)
            else:
                raise RealmoceanException(e)
    ...
    def chunked_upload(
        ...
        while offset < size:
            ...
            if "$id" in result: 
                headers["x-realmocean-id"] = result["$id"]
